...
const Header = ({
  ...
  useEffect(() => {
    // Set CS2 categories
    const cs2Categories = headerData.games.find(
      (game) => game.name === "CS2"
    ).categories;
      ...
      <div className="w-full flex lg:px-4  px-1 lg:py-2  flex-col headerBg ">
        <header className="border-none  lg:border-b pb-2 center w-full  border-gray-600 ">
          ...
          <div className=" hidden md:flex justify-center items-center mx-2  ">
            <Link to='/signin' className="text-white p-3 mx-2 hover:text-navColor">
              SignIn
            </Link>
            <Link to='/signin' className="text-white flex items-center lg:px-3 lg:py-1 mx-2 text-[15px] px-3  h-9 btnBg rounded-md shadow-md  ">
              SignUp
            </Link>
          </div>
          {/* mobile layout  */}
          <div className="md:hidden block">
            ...
            {isSearchHide && (
              <div
                className={`mblSearch  bg-black absolute top-0 left-0 z-200 md:hidden flex p-3 space-x-2   justify-center items-center  ${
                  isSearchVisible
                    ? "animated-search-bar"
        ...
        <div className="mt-2 w-full    transition-all  lg:flex justify-center items-center  hidden relative secondHeaderBg">
                ...
                return (
                  <li
                    ...
                    className={`px-3 hover:text-red-500 m-1 cursor-pointer whitespace-nowrap uppercase font-bold text-sm leading-tight tracking-wider ${
                      activeTab && activeTab.name === category.name
                        ? "text-navColor"
                        : "text-white"
                    }`}
                  >
          ...
          {showMoreTabs && (
            <ul className="absolute flex top-12 bg-[#26213a] right-0">
              {overflowTabs.map((category) => (
                <li
                  key={category.name}
                  onClick={() => handleTabClick(category)}
                  className="text-white px-3 m-1 whitespace-nowrap uppercase font-bold text-sm leading-tight tracking-wider"
                >
                  <h3>{category.name}</h3>
                </li>
              ))}
          ...
          {activeTab && (
            <div
              className="absolute  z-30 top-10 pb-10 text-white tabsBg w-full"
              style={{
